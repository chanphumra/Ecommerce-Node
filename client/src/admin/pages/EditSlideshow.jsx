...
const EditSlideshow = () => {
    ...
    const getSlideshow = async () => {
        const url = 'http://localhost:8000/api/slideshow/'+id;
        const res = await axios.get(url);
        const data = res.data.list;
        titleRef.current.value = data[0].title;
        titleRef.current.value = data[0].title;
        textRef.current.value = data[0].text;
        linkRef.current.value = data[0].link;
        enableRef.current.checked = data[0].enable == 1;
    ...
    const update = async () => {
        ...
        if(title != '' && text != '' && link != ''){
            ...
            enableRef.current.checked = false; setImage('');
    ...
    return (
        <div className='lg:py-7 lg:px-10 p-5'>
            <div className="flex justify-between items-end">
                <h1 className='text-3xl font-bold text-black_500'>Edit slideshow</h1>
                <button onClick={() => update()} className='px-4 py-2 rounded-md bg-primary text-white text-sm cursor-pointer'>Publish slideshow</button>
            </div>
