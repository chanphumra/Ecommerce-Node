...
const SubCategoryTable = () => {
    ...
    const main_id = useParams().id;

    const getSubcategory = async () => {
        await axios.get(
            'http://localhost:8000/api/subcategory/main/'+ main_id
        ).then(res => {
    ...
    const deleteSubcategory = async (props) => {
        await axios.get(
            ...
            if (count <= 0) {
                await axios.delete(
                    'http://localhost:8000/api/subcategory/' + props.id
                );
                                ...
                                subcategory.map((item, index) => {
                                    if (index >= (activePage - 1) * ITEM_PER_PAGE && index < activePage * ITEM_PER_PAGE) {
                                        return <tr key={index} className='group relative border-solid border-b border-gray-300' >
                                            ...
                                            {/* <td className='text-ph font-semibold text-gray-700 py-2 px-3'>
                                                <p>{subcategory.counts[index][0].count}</p>
                                            </td> */}
                                            ...
                                            <div className='hidden group-hover:flex absolute right-0 top-[50%] translate-y-[-50%] pr-[10px]  gap-1'>
                                                ...
                                                <Link to={`/admin/edit_subcategory/${item.id}?page_name=show_category`}>
                                                    <div className='inline-flex px-[10px] py-[6px] bg-body border-solid border border-gray-300 rounded-md cursor-pointer hover:bg-gray-200'>
                                                        <MdEdit />
                                                    </div>
                                                </Link>
                                                <div onClick={() => deleteSubcategory({ id: item.id })} className='inline-flex px-[10px] py-[6px] bg-body border-solid border border-gray-300 rounded-md cursor-pointer hover:bg-gray-200'>
                                                    <IoMdTrash />
                                                </div>
                                            </div>
                                        </tr>
                    ...
                    <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                        <div>
                            <p className="text-sm text-gray-700">
                                Showing <span className="font-medium">{`${subcategory.length > 0 ? (activePage - 1) * ITEM_PER_PAGE + 1: 0}`}</span> to <span className="font-medium">{(subcategory.length - (activePage - 1) * ITEM_PER_PAGE) > ITEM_PER_PAGE ? activePage * ITEM_PER_PAGE : subcategory.length}</span> of{' '}
                                <span className="font-medium">{subcategory.length}</span> results
                            </p>
                        </div>
                        <div>
