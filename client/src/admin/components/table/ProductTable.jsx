...
const header = ['PRODUCT NAME', 'PRICE', 'IN STOCK','DISCOUNT', 'CATEGORY', 'SUB CATEGORY', 'PUBLISHED ON'];
...
const ProductTable = () => {
    ...
    const deleteProduct = async (props) => {
        await axios.delete(
            'http://localhost:8000/api/product/' + props.id
        );
        getProducts();
        if ((products.length - 1)  % 5 == 0) {
        ...
        <>
            <div className="flex gap-4 flex-col lg:flex-row lg:items-center justify-between px-5 lg:px-10 mb-4">
                <div className='relative'>
                    <input type="text" name="" id="" placeholder='Search products' className='text-sm pl-10 w-full lg:w-[300px] h-[40px] rounded-md border-gray-300 border-solid border focus:border-current focus:ring-current' />
                    <RiSearchLine className='absolute top-[50%] left-3 translate-y-[-50%] w-4 h-4' />
                </div>
                <Link to="/admin/add_product?page_name=add_product">
                    <button className='flex items-center gap-1 px-4 py-2 rounded-md bg-primary text-white text-sm cursor-pointer'><AiOutlinePlus className='text-white text-lg' />Add product</button>
                </Link>
            </div>
                                    ...
                                    if (index >= (activePage - 1) * ITEM_PER_PAGE && index < activePage * ITEM_PER_PAGE) {
                                        return <tr className='group relative border-solid border-b border-gray-300' >
                                            ...
                                            <td className='py-2 px-3 flex items-center gap-1'>
                                                ...
                                                <p className='w-[230px] text-ph font-semibold truncate text-primary py-2 px-3 cursor-pointer hover:underline'>{item.p_name}</p>
                                            </td>
                                            <td className='text-ph font-semibold text-gray-700 py-2 px-3'>${item.sale_price}</td>
                                            <td className='text-ph font-semibold text-gray-700 py-2 px-3'>{item.qty}</td>
                                            <td className='text-ph font-semibold text-gray-700 py-2 px-3'>{item.discount}%</td>
                                            <td className='text-ph font-semibold text-gray-700 py-2 px-3'>{item.main_name}</td>
                                            <td className='text-ph font-semibold text-gray-700 py-2 px-3'>{item.sub_name}</td>
                                            ...
                                            <div className='hidden group-hover:flex absolute right-0 top-[50%] translate-y-[-50%] pr-[10px]  gap-1'>
                                                ...
                                                <div onClick={()=>{deleteProduct({id:item.p_id})}} className='inline-flex px-[10px] py-[6px] bg-body border-solid border border-gray-300 rounded-md cursor-pointer hover:bg-gray-200'>
                                                    <IoMdTrash />
                                                </div>
                                            </div>
                                        </tr>
