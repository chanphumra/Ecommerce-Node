...
const OrdersTable = () => {
    ...
    return (
        <>
            <div className='bg-white relative border-solid border-t shadow-sm border-b border-gray-200 border-t-gray-300 rounded-md'>
                <div className="w-full pb-2 rounded-md overflow-x-auto overflow-y-none scroll-smooth scrollbar-thin scrollbar-track-gray-200 scrollbar-track-rounded-xl scrollbar-thumb-current scrollbar-thumb-rounded-xl ">
                    <table className='w-[800px] md:w-full rounded-md'>
                                    ...
                                    if (index >= (activePage - 1) * ITEM_PER_PAGE && index < activePage * ITEM_PER_PAGE) {
                                        return <tr key={index} className='group relative border-solid border-b border-gray-300' >
                                            ...
                                            <td className='text-ph font-semibold text-primary hover:underline cursor-pointer py-2 px-3'>
                                                <Link to={`/orderdetails/${item.id}?page_name=orders`}>#{item.id}</Link>
                                            </td>
                ...
                {/* pagination */}
                <div className="flex items-center justify-between border-t border-gray-200 bg-white px-2 py-3 sm:px-2 rounded-md">
