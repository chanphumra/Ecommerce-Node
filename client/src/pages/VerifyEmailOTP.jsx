...
const VerifyEmailOTP = () => {
    ...
    const verifyEmail = async () => {
        if (num1.current.value === '' && num2.current.value === '' && num3.current.value === '' && num4.current.value === '')
            return;
        const OTP = num1.current.value + num2.current.value + num3.current.value + num4.current.value;

        const url = "http://localhost:8000/api/customer/verify/" + email;
        const res = await axios.get(url);
        const data = res.data.list;
        if (data.length > 0) {
            if (OTP === data[0].otp) {
                // upadate verify in table customer
                await axios.put('http://localhost:8000/api/customer', { emailReg: email, verify: 1 });
                // delete when verify success
                await axios.delete('http://localhost:8000/api/customer/email/' + email);
                // goto login page
    ...
    const resendOTP = async () => {
        ...
        await axios.post('http://localhost:8000/api/customer/resend', {email: email, OTP: OTP});
